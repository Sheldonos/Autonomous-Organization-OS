...
1. `WebBrowser.openAuthSessionAsync()` opens Manus OAuth

1. User authenticates

1. Deep link redirects to `app/oauth/callback.tsx`

1. Callback exchanges code for session token

1. Token stored in SecureStore
...
1. Browser redirects to Manus OAuth

1. User authenticates

1. Redirect back with session cookie

1. Cookie automatically sent with requests
...
1. Handle the boolean return (`true` on success, `false` if the upstream service is temporarily unavailable) to decide whether you need a fallback channel.
