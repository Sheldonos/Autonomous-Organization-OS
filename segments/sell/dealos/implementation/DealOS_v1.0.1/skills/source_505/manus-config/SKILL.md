...
| Domain | Scope | CLI |
| --- | --- | --- |
| Connector | Current task | `manus-config config load |
| Project | All tasks in the project, for project tasks only | `manus-config config load |
| Schedule | One schedule per current task; survives until disabled or expired | `manus-config schedule create |
...
**Critical workflow: always ****`load`**** → edit → ****`save`****.** Before starting any new edit session, MUST run `load` first to refresh the baseline. Do not re-run `load` *after* starting edits (mid-edit) unless intentionally discarding them. A stale baseline causes `save` to produce incorrect diffs.
