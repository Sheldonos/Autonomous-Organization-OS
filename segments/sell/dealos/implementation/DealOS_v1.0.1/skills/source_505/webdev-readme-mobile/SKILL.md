...
1. **Customize theme** — Update tokens in `theme.config.js` (used by Tailwind + runtime) and app details in `app.config.ts`

1. **Add new screens** — Use `ScreenContainer` component for proper SafeArea handling

1. **Add tab icons** — Map icons in `icon-symbol.tsx` BEFORE using in tabs
...
1. **Feedback** — User knows their action was received (press states, loading indicators)

1. **Polish** — Animations and transitions (only if time permits)
